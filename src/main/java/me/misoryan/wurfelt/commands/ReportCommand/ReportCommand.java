    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if (command.getName().equalsIgnoreCase("wdr") || command.getName().equalsIgnoreCase("report")) {
            ...
            if (Bukkit.getPlayerExact(strings[0]) != null) {
                player = Objects.requireNonNull(Bukkit.getPlayerExact(strings[0])).getName();
            } else {
                player = strings[0];
            }
            String reason = Lib.getCurrentArgsFormat(strings, 1);
            ReportUtils.createNewReport(player, reason, commandSender.getName());
            commandSender.sendMessage(Lib.getCurrentText(Objects.requireNonNull(Wurfelt.ins.getConfig().getString("report.report-success")).replace("%player%", strings[0])));
            return true;
        }
        return true;
    }
